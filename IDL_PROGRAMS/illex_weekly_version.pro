...
; ****************************************************************************************************
  ...
  FOR V=0, N_ELEMENTS(VERSION)-1 DO BEGIN
    ...
    FOR P=0, N_ELEMENTS(PRODS)-1 DO BEGIN
      SPROD = PRODS[P]
      CASE SPROD OF
        'CHLOR_A':  BEGIN & DTSET=CHL_DATASET & SPROD=SPROD+'-'+CHL_ALG & DWLPROD='CHL1' & PTAG='MED'   & PSCALE='CHLOR_A_0.01_10' & PAL='PAL_NAVY_GOLD'  & ASCALE='RATIO'    & APAL='PAL_BLUEGREEN_ORANGE' & END
        'SST':      BEGIN & DTSET=SST_DATASET & SPROD=SPROD             & DWLPROD='' & PTAG='AMEAN' & PSCALE='SST_0_30'       & PAL='PAL_BLUE_RED'   & ASCALE='DIF_-5_5' & APAL='PAL_ANOM_BWR'    & END
        'GRAD_CHL': BEGIN & DTSET=GRADCHL_DATASET & SPROD=SPROD+'-'+GCHL_ALG & DWLPROD='' & PTAG='' & PSCALE='' & PAL='' & ASCALE='' & APAL='' & END
        'GRAD_SST': BEGIN & DTSET=GRADSST_DATASET & SPROD=SPROD+'-'+GSST_ALG & DWLPROD='' & PTAG='' & PSCALE='' & PAL='' & ASCALE='' & APAL='' & END
      ENDCASE ; SPROD
      STR = CREATE_STRUCT('DATASET',DTSET,'PROD',SPROD,'DOWNLOAD_PROD',DWLPROD,'PLOT_TAG',PTAG,'PROD_SCALE',PSCALE,'PAL',PAL,'ANOM_SCALE',ASCALE,'ANOM_PAL',APAL)
