  ...
  FOR V=0, N_ELEMENTS(VERSION)-1 DO BEGIN
    ...
    IF KEYWORD_SET(PROCESS_FILES) THEN BEGIN ;ILLEX_WEEKLY_PROCESSING, VERSTR
      ...
      FOR N=0, N_ELEMENTS(PRODS)-1 DO BEGIN
        ...
        STACKED_STATS_WRAPPER, DSET, PRODS=PSTR.PROD, PERIODS=VERSTR.INFO.STAT_PERIODS                        & IF TSET NE [] THEN STACKED_STATS_WRAPPER, TSET, PRODS=PSTR.PROD, PERIODS=VERSTR.INFO.STAT_PERIODS
        STACKED_ANOMS_WRAPPER, DSET, PRODS=PSTR.PROD, PERIODS=VERSTR.INFO.ANOM_PERIODS                        & IF TSET NE [] THEN STACKED_ANOMS_WRAPPER, TSET, PRODS=PSTR.PROD, PERIODS=VERSTR.INFO.ANOM_PERIODS
    ...
    IF KEYWORD_SET(ANIMATIONS) THEN BEGIN
     ; ILLEX_VIEWER_ANIMATION, VERSTR, MAPP=MAPP, BUFFER=BUFFER, /ADD_CONTOURS, /ADD_COLORBAR, /ADD_DATEBAR, /ADD_BATHY, /ADD_LONLAT, /ADD_BOARDER;, OUTCOLOR=0
      YR = VERSTR.INFO.YEAR
      MONTHS = MONTH_RANGE('01',DP.MONTH,/STRING)
      DATERANGES = LIST(GET_DATERANGE(YR+MONTHS[M]+'01',YR+MONTHS[M]+DAYS_MONTH(MONTHS[M],/STRING))) ; LIST(GET_DATERANGE(VERSTR.INFO.YEAR))
      FOR M=1, N_ELEMENTS(MONTHS)-1 DO DATERANGES.ADD,GET_DATERANGE(YR+MONTHS[M]+'01',YR+MONTHS[M]+DAYS_MONTH(MONTHS[M],/STRING))
      ...
      FOR D=9, N_ELEMENTS(DATERANGES)-1 DO BEGIN
        DTR = DATERANGES[D]
        PROJECT_MAKE_ANIMATION, VERSTR, DATERANGE=DTR,MAPP=MAPP, BUFFER=BUFFER,OUTLINE=CANYON_OUTLINES, /ADD_CONTOURS, /ADD_COLORBAR, /ADD_DATEBAR, /ADD_BATHY, /ADD_LONLAT, /ADD_BOARDER, OUTCOLOR=0
      ENDFOR
    ENDIF
    IF KEYWORD_SET(MAKE_COMPOSITES) THEN $
      PROJECT_MAKE_COMPOSITE, VERSTR, PRODS=['SST','CHLOR_A'],/WEEKS,OUTLINE=CANYON_OUTLINES,/ADD_LONLAT,/ADD_COLORBAR,/ADD_BATHY,/ADD_BOARDER,OUT_COLOR=0,BUFFER=1
