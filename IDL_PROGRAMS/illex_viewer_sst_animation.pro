; $ID:	ILLEX_VIEWER_SST_ANIMATION.PRO,	2022-08-08-09,	USER-KJWH	$
  PRO ILLEX_VIEWER_SST_ANIMATION, VERSTRUCT, NDAYS=NDAYS, WEEKS=WEEKS, MAPP=MAPP, PROD_SCALE=PROD_SCALE, BUFFER=BUFFER, DIR_OUT=DIR_OUT, _REF_EXTRA=EXTRA
...
;
; CALLING SEQUENCE:
;   ILLEX_VIEWER_SST_ANIMATION,$Parameter1$, $Parameter2$, $Keyword=Keyword$, ....
;
; REQUIRED INPUTS:
...
;-
; ****************************************************************************************************
  ROUTINE_NAME = 'ILLEX_VIEWER_SST_ANIMATION'
  COMPILE_OPT IDL2
  SL = PATH_SEP()
  ...
  FOR N=0, N_ELEMENTS(PRODS)-1 DO BEGIN
    ...
    FOR M=0, N_ELEMENTS(MDATES)-1 DO BEGIN
      ...
      WEEKS = REVERSE('W_' + VSTR.INFO.ILLEX_YEAR + WKS)
      FOR W=0, N_ELEMENTS(WEEKS)-1 DO BEGIN
        ...
        FOR PNF=0, N_ELEMENTS(DATES)-1 DO BEGIN
          PNGFILE = DIR_PNGS + REPLACE(FP.NAME +'.PNG',[FP.PERIOD,FP.PROD,FP.MAP],[DATE_2PERIOD(STRMID(DATES[PNF],0,8)),PSCL,MP])
          PNGFILE = REPLACE(PNGFILE, '-'+['STACKED','STACKED_STATS','STACKED_ANOMS'],['','',''])
          PNGFILES = [PNGFILES,PNGFILE]
        ENDFOR
        FA = PARSE_IT(PNGFILES[0])
        OUTPERIOD = 'DD_'+ STRMID(DATES[0],0,8) +  STRMID(DATES[-1],0,8)
        ...
        MFILE = DIR_OUT + REPLACE(FA.NAME,FA.PERIOD,OUTPERIOD)
        EXT = ['webm'] ; 'mov','mp4',
        PNG_OVERWRITE = 0
