; $ID:	ILLEX_VIEWER_IMAGE.PRO,	2022-08-08-09,	USER-KJWH	$
  FUNCTION ILLEX_VIEWER_IMAGE, VERSTR, FILE=FILE, MAPP=MAPP, BUFFER=BUFFER, CURRENT=CURRENT, $
                               ...
                               ADD_POINTS=ADD_POINTS, PLONS=PLONS, PLATS=PLATS, PSYM=PSYM, PFILLED=PFILLED, PTHICK=PTHICK, PCOLOR=PCOLOR, PSIZE=PSIZE, PTRANSPARENCY=PTRANSPARENCY,$
...
if prod eq 'TOPO' then prod = 'BATHY'  
  ...
  PRODS_2PNG, FILE, OBJ=IMG, MAPP=MP, PAL=PAL, PROD=PROD, SPROD=PSCL, ADD_BATHY=BATHY, OUTLINE=OUTLINE, OUT_COLOR=OUT_COLOR, OUT_THICK=OUT_THICK, $
  ...
  IF KEYWORD_SET(ADD_POINTS) THEN BEGIN
    IF IDLTYPE(PLONS) EQ 'OBJREF' THEN BEGIN
      FOR PL=0, N_ELEMENTS(PLONS)-1 DO BEGIN
        PLON = PLONS[PL]
        PLAT = PLATS[PL]
        MAPS_SET, MP
        LL = MAP_DEG2IMAGE(MAPS_BLANK(MP),PLON,PLAT,X=PX,Y=PY)
        ZWIN
        PSY = SYMBOL(PX*XX,PY*XX,SYMBOL=PSYM[PL],/DEVICE, SYM_FILLED=PFILLED[PL], SYM_THICK=PTHICK[PL], SYM_COLOR=PCOLOR[PL], SYM_SIZE=PSIZE[PL], SYM_TRANSPARENCY=PTRANSPARENCY[PL],TARGET=IMG)       
        IF KEYWORD_SET(RESIZE) AND KEYWORD_SET(ADD_POINTS) THEN PSY.SCALE,RESIZE,RESIZE
    ...
    ENDIF ELSE BEGIN
      ...
      ZWIN
      PSY = SYMBOL(PX*XX,PY*XX,SYMBOL=PSYM,/DEVICE, SYM_FILLED=PFILLED, SYM_THICK=PTHICK, SYM_COLOR=PCOLOR, SYM_SIZE=PSIZE, SYM_TRANSPARENCY=PTRANSPARENCY, TARGET=IMG)
      IF KEYWORD_SET(RESIZE) AND KEYWORD_SET(ADD_POINTS) THEN PSY.SCALE,RESIZE,RESIZE
    ENDELSE  
  ENDIF
  ...
  IF KEYWORD_SET(ADD_COLORBAR) THEN CBAR, PSCL, IMG=IMG, FONT_SIZE=MR.CB_SIZE, CB_TYPE=FIX(MR.CB_TYPE), CB_POS=CB_POS, CB_TITLE=CBTITLE, CB_TICKNAMES=CB_TICKNAMES, CB_TICKVALUES=CB_TICKVALUES, PAL=PAL, RELATIVE=CB_RELATIVE, CB_OBJ=CB_IMG, _EXTRA=EXTRA
  IF KEYWORD_SET(ADD_DATEBAR) THEN DB = DATE_BAR(FP.DATE_START,DB_FONT_SIZE=18,DB_COLOR=DB_COLOR,DB_THICK=5,DB_POS=DB_POS,DB_BKG=DB_BKG, PAL=PAL)
