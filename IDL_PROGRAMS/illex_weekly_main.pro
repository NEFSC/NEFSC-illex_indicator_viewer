        ...
        FOR N=0, N_ELEMENTS(PRODS)-1 DO BEGIN
          ...
          FOR T=0, N_ELEMENTS(TYPES)-1 DO BEGIN
            ...
            FOR F=0, N_ELEMENTS(FILES)-1 DO BEGIN
              ...
              W = WINDOW(DIMENSIONS=IMG_DIMS,BUFFER=BUFFER)
              PRODS_2PNG, FILES[F],PAL=PAL, SPROD=PSCL, ADD_BATHY=200, CB_TITLE=CBTITLE, OBJ=IMG, LAYOUT=[2,2,1], OUTLINE=CANYON_OUTLINES,OUT_COLOR=OCOLOR,OUT_THICK=OTHICK, $
...
;      ENDFOR
   ...
   ;     DIR_GIF=VERSTR.DIRS.DIR_ANIMATIONS + PRODS[N] + SL + 'ANIMATION_GIFS' + SL & DIR_TEST, DIR_GIF
    ...
    ;        GFILE   = DIR_GIF + REPLACE(FP.NAME +'.gif',FP.MAP,MP)
            ...
            IF ~FILE_MAKE(FILES[F],[PNGFILE],OVERWRITE=OVERWRITE,VERBOSE=VERBOSE) THEN CONTINUE
BUFFER = 1          
IMG = ILLEX_WEEKLY_IMAGE(VERSTR, FILE=FILES[F],BUFFER=BUFFER,OUTLINE=CANYON_OUTLINES, /ADD_CONTOURS, OUT_COLOR=251, /ADD_BATHY, /ADD_LONLAT, PSCL=PSCL, CBTITLE=CBTITLE, SCLR=SCLR, PAL=PAL, LONS=LONS, LATS=LATS)
...
SY = SYMBOL(X*XX,Y*XX,SYMBOL='CIRCLE',/DEVICE, /SYM_FILLED, SYM_COLOR=SCLR, SYM_SIZE=0.65, TARGET=IMG)
            ...
            LAT = FLOAT(WHERE_NEAREST(MLL.LATS[0,*],FLOAT(LATS),NEAR=0.05))*XX & LATX = REPLICATE(PIXBL[1],N_ELEMENTS(LAT))
            LON = FLOAT(WHERE_NEAREST(MLL.LONS[*,0],FLOAT(LONS),NEAR=0.05))*YY & LONY = REPLICATE(PIXBL[0],N_ELEMENTS(LON))
            
           
            LLAT = SYMBOL(LATX,LAT,'HLINE',/DEVICE,SYM_SIZE=2,SYM_COLOR='BLACK',SYM_THICK=1,/SYM_FILLED,/CURRENT,LABEL_STRING=LATNAMES,LABEL_FONT_SIZE=14,LABEL_POSITION='L',TARGET=IMG)
            LLON = SYMBOL(LON,LONY,'VLINE',/DEVICE,SYM_SIZE=2,SYM_COLOR='BLACK',SYM_THICK=1,/SYM_FILLED,/CURRENT,LABEL_STRING=LONNAMES,LABEL_FONT_SIZE=14,LABEL_POSITION='B',TARGET=IMG)

            ;FOR S=1, N_ELEMENTS(LATS)-1 DO LLAT = SYMBOL(X2,LAT[S]+SP, 'HLINE',SYM_SIZE=2,SYM_COLOR='BLACK',SYM_THICK=3,/SYM_FILLED,/CURRENT,/DEVICE,LABEL_STRING=LATNAMES[S],LABEL_FONT_SIZE=14,LABEL_FONT_STYLE='',LABEL_POSITION='R')
            ;FOR S=0, N_ELEMENTS(LONS)-1 DO LLON = SYMBOL(LON[S]+SP,Y2, 'VLINE',SYM_SIZE=2,SYM_COLOR='BLACK',SYM_THICK=3,/SYM_FILLED,/CURRENT,/DEVICE,LABEL_STRING=LONNAMES[S],LABEL_FONT_SIZE=14,LABEL_FONT_STYLE='',LABEL_POSITION='T')
