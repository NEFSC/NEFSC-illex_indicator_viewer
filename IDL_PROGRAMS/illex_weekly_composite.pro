...
PRO ILLEX_WEEKLY_COMPOSITE, VERSTR, PRODS=PRODS, TYPES=TYPES, MAPP=MAPP, WEEKS=WEEKS, DAYS=DAYS, DATERANGE=DATERANGE, BUFFER=BUFFER, CURRENT=CURRENT, DIR_OUT=DIR_OUT, $
...
;
; CALLING SEQUENCE:
;   ILLEX_WEEKLY_COMPOSITE($Parameter1$, $Parameter2$, $Keyword=Keyword$, ...)
;
; REQUIRED INPUTS:
...
;-
; ****************************************************************************************************
  ROUTINE_NAME = 'ILLEX_WEEKLY_COMPOSITE'
  COMPILE_OPT IDL2
  SL = PATH_SEP()
...
;  PER = PERIOD_2STRUCT(FP.PERIOD)
  ...
  IF ~N_ELEMENTS(TYPES) THEN TYPS = ['STATS','ANOMS']     ELSE TYPS = TYPES
  ...
  FOR K=0, N_ELEMENTS(PERS)-1 DO BEGIN
    ...
    CASE PER.PERIOD_CODE OF
      ...
      'D': BEGIN & TXT = (DATE_PARSE(PER.DATE_START)).DASH_DATE & TPS = 'SAVE' & END
    ENDCASE  
    ...
    FOR N=0, N_ELEMENTS(PRDS)-1 DO BEGIN
      ...
      FOR T=0, N_ELEMENTS(TPS)-1 DO BEGIN
        ATYPE = TPS[T] 
        FILE = GET_FILES(DSET,PRODS=PSTR.PROD,PERIOD=PER.PERIOD_CODE,FILE_TYPE=ATYPE, DATERANGE=DR)
    ...
    FOR N=0, N_ELEMENTS(TPS)-1 DO BEGIN
      ...
      FOR T=0, N_ELEMENTS(PRDS)-1 DO BEGIN
        ...
        IMG = ILLEX_WEEKLY_IMAGE(VERSTR, FILE=FILE, BUFFER=BUFFER, RESIZE=RESZ, MAPP=MP, PROD_SCALE=PROD_SCALE, _EXTRA=EXTRA)
