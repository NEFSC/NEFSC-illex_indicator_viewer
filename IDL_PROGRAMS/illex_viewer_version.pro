...
; ****************************************************************************************************
  ...
  FOR V=0, N_ELEMENTS(VERSION)-1 DO BEGIN
    ...
    ISTR = CREATE_STRUCT('DATAFILE',DATFILE,'ILLEX_YEAR',ILLEX_YR,'DATERANGE',DATERANGE,'PREVIOUS_DATERANGE',PREVIOUS_DATERANGE,'FULL_DATERANGE',FULL_DATERANGE,'MAP_OUT',MAP_OUT,'RESOLUTION',RESOLUTION, $
      'STAT_PERIODS',STAT_PERIODS,'ANOM_PERIODS',ANOM_PERIODS,'PNG_PRODS',PNG_PRODS,'PNG_PERIODS',PNG_PERIODS,'ANIMATION_PRODS',ANIMATION_PRODS,'NETCDF_PRODS',NETCDF_PRODS,'NETCDF_PERIODS',NETCDF_PERIODS,'DOWNLOAD_PRODS',DOWNLOAD_PRODS,'PROCESS_PRODS',PROCESS_PRODS,'TEMP_PRODS',TEMP_PRODS,'FRONT_PRODS',FRONT_PRODS,'EXTRACT_PRODS',EXTRACT_PRODS,'EXTRACT_PERIODS',EXTRACT_PERIODS,'GRID_PERIOD',GRID_PERIOD)
    ...
    FOR P=0, N_ELEMENTS(PRODS)-1 DO BEGIN
      ...
      MONTH_SCALE = []
      CASE SPROD OF
        'CHLOR_A':  BEGIN & DTSET=CHL_DATASET & TPSET=CHL_TEMP & SPROD=SPROD+'-'+CHL_ALG & DWLPROD='CHL1' & PTAG='MED'   & PSCALE='CHLOR_A_0.1_10' & PAL='PAL_NAVY_GOLD'  & ASCALE='RATIO'    & APAL='PAL_BLUEGREEN_ORANGE' & ATITLE='Chlorophyll Anomaly (Ratio)' & END
